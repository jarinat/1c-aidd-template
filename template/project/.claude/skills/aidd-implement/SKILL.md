...
- Один запуск `aidd-implement` обрабатывает все незакрытые блоки `tasklist`
  последовательно до конца.
- Молчаливая остановка после N блоков запрещена. Допустимые причины остановки
  до конца `tasklist`: blocker без технически безопасного решения; текущий блок
  явно требует участия пользователя; пользователь явно попросил checkpoint;
  все блоки закрыты.
- Если основная сессия считает нужным внеплановый checkpoint, сначала получи
  явное решение пользователя с конкретным обоснованием. Не оформляй такой
  checkpoint как финальный отчёт и не рационализируй остановку задним числом
  через "осторожность", "бюджет контекста" или "удобный момент".
...
3. Прочитай PRD, plan, tasklist, `research` и `feedback`, если они существуют.
...
7. Для каждой незакрытой задачи выбранного блока запусти отдельного
   ...
   - явно укажи `implementer`, что новые локальные имена кода не должны
     сокращать объекты метаданных или их элементы: процедуры, функции,
     helper'ы, переменные, параметры, поля структур, fixture-поля и тестовые
...
8. После каждой задачи проверь результат и отсутствие выхода за scope.
   ...
   реализации, и подготовь запись для `tasklist` текущего блока. Если
   наблюдение относится к выбранному EDT MCP, до fallback или следующей задачи зафиксируй
   краткий факт в tasklist и каноническую запись в research, если он есть,
   иначе в PRD, по artifacts rule.
...
10. Сразу после успешного commit блока или успешной verification-only проверки
   ...
   `Tooling gaps`, не меняя scope и acceptance criteria задним числом. Для
   EDT MCP добавь ссылку на каноническую запись research или PRD, а не дублируй
   evidence.
