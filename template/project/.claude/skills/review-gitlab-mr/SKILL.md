...
Чтение уже существующих замечаний MR, ответы в треды ревьюера и resolve — это
отдельный сценарий `.claude/skills/gitlab-tools/SKILL.md` и отдельный script
`.claude/scripts/gitlab-tools.cmd`. Не добавляй такие команды в
