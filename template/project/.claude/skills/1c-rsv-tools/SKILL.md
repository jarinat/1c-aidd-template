...
### Безопасная запись больших СКД

Для большой или сложной `Template.dcs`, особенно при перезаписи запроса или
набора данных, проверяй не только синтаксис, но и структурную целостность:

1. До первой записи зафиксируй scoped `git status`/`git diff` и структурные
   инварианты схемы: количество `field`, `parameter`, `calculatedField`,
   `totalField`, `settingsVariant` и других значимых секций.
2. После каждой write-операции повторно проверь инварианты и scoped diff. Не
   откладывай проверку до конца серии правок.
3. Не считай `success:true` или отсутствие ошибок `get_validation_errors`
   доказательством сохранности структуры: EDT-маркеры могут не обнаружить
   удалённые поля, параметры и настройки схемы.
4. Если файл был изменён внешним процессом либо исчезли несвязанные секции,
   прекрати дальнейшие записи, сохрани evidence и зафиксируй tooling gap.
   Восстанавливай файл из заведомо корректного состояния только после проверки
   и сохранения несвязанных пользовательских изменений.
