...
name: rlm-tools-bsl
description: >
  Read-only исследование 1С/EDT/BSL кодовых баз через MCP-сервер
  rlm-tools-bsl. В этом шаблоне 1С/EDT проекты считаются большими по умолчанию.
  Используй как основной слой discovery/research/review, когда
  нужно найти объекты метаданных, методы, формы, ссылки, движения, подписки,
  интеграции или локальные reference patterns.
---
...
- Исследование EDT/CF-кодовой базы на этапе `research`.
...
В этом шаблоне любая 1С/EDT кодовая база считается большой по умолчанию:
неизвестные связи и широкий поиск сначала сужай через MCP helpers.
...
3. Сначала проверь индекс через
...
- Для broad search предпочитай `find_module`, `search_*`,
  `find_references_to_object` и специализированные helpers вместо широкого
  `grep` по всему дереву.
